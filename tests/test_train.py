...
from librubiks.solving.agents import ValueSearch
...
class TestTrain(MainTest):
	...
	def test_train(self):
		...
		train = Train(rollouts=2, batch_size=2, tau=0.1, alpha_update = .5, gamma=1, rollout_games=2, rollout_depth=3, optim_fn=torch.optim.Adam, agent=ValueSearch(None), lr=1e-6, evaluation_interval=1, evaluator=evaluator, update_interval= 1, with_analysis=True, reward_method='schultzfix')
