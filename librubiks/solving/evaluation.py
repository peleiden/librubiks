...
class Evaluator:
    ...
    def eval(self, agents: List[Agent]) -> EvalData:
        ...
        time_used = []
        for agent in agents:
            log.section(f"Evaluating {agent}")
            log("\n".join([
    ...
    def log_this_depth(self, res: np.ndarray, states: np.ndarray, times: np.ndarray, depth: int):
        ...
        won_games = res[res != -1]
        log(f"Scrambling depth {depth if depth else 'deep'}", with_timestamp=False)
        log(
        ...
        log(f"\tTime:  {times.mean():.2f} +/- {times.std():.2f} (std.)", with_timestamp=False)
